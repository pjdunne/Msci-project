...
vector<Particle> threshold(vector<Particle> PartVec, float prot_thresh, float neutron_thresh, float pi_pm_thresh, float Eff){
  ...
  for(unsigned int i=0;i < PartVec.size();i++){
    ...
    else if (abs(pdg) != 211 && abs(pdg) != 2212 && abs(pdg) != 2112 && abs(pdg) != 12 && abs(pdg) != 14 && r < Sigmoid(pmag, 0, Eff)) {
      PartVecAbove.push_back(Part);     
    }
  }
...
vector<Particle> liquidMomThresh(vector<Particle> PartVec){
  ...
  return threshold(PartVec, prot_thresh, neutron_thresh, pi_pm_thresh, Eff);
