...
int readenergy(){
  TFile* file = new TFile("/Users/AnishaKadri/Desktop/code2/Eshita.root");
  ...
  for(unsigned int iEntry=0;iEntry< nentries;iEntry++){
    ...
    //Applying resolution functions:
    //PartVec = resolution(PartVec,0.5);

    //Applying gas and liquid threshold functions:
    PartVecAboveLiquid = liquidMomThresh(PartVec);
    PartVecAboveGas = gasMomThresh(PartVec);

    //Calorimetric Reconstruction for liquid:
    float ECL = calorimetric(PartVecAboveLiquid);
    float ECL_diff = ECL-Enu_t;
    hCL->Fill(ECL);
    hCL_diff->Fill(ECL_diff);

    //Calorimetric Reconstruction for gas:
    ...
    if(mode==1){ //CCQE mode selection-> change to slection by pdg

      //Kinematic Reconstruction for liquid:
      float EKL = kinematic(PartVecAboveLiquid,coslep);
      float EKL_diff = EKL - Enu_t;
      hKL->Fill(EKL);
      hKL_diff->Fill(EKL_diff);

      //Kinematic Reconstruction for gas:
